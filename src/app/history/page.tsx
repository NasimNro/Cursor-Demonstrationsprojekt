...
export default function HistoryPage() {
  ...
  const handleFormSubmit = (data: { weight: number; date: string; notes: string }) => {
    updateWeight(data);
  };
  ...
  return (
    <main className="min-h-[100svh] text-gray-100">
            ...
            {weights.length === 0 ? (
              <div className="bg-[#161618] p-4 sm:p-6 rounded-2xl shadow-md text-center border border-white/5">
                <p className="text-gray-300">
                  No weight entries yet. Add your first entry on the dashboard!
                </p>
                <Link
                  href="/"
                  className="mt-4 inline-block px-6 py-3 bg-gradient-to-r from-blue-500 to-blue-400 text-white rounded-full shadow-[0_0_20px_rgba(59,130,246,0.3)]"
                >
                  Go to Dashboard
                </Link>
              </div>
      ...
      <Modal isOpen={isModalOpen} onClose={handleCloseModal} title="Edit Weight Entry">
        <WeightForm
